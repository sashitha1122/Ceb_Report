...
{
    public class CalCycleRepository : ICalCycleRepository
    {
        private readonly string _connectionString;

        public CalCycleRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("Informix_billsmry")!;
        }

        public async Task<IEnumerable<CalCycleInfo>> GetLast24CalCyclesAsync()
        {
            var result = new List<CalCycleInfo>();
            int maxCycle = 0;
            ...
            try
            {
                ...
                using var command = new IfxCommand("SELECT MAX(calc_cycle) FROM netmtcons", connection);
